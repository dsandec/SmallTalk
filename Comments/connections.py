...
#define class by extending TypeDecorator
class TextPickleType(TypeDecorator):
    #instantiate impl as sqlalchemy.Text(256)
	impl = sqlalchemy.Text(SIZE)

	def process_bind_param(self, value, dialect):
		if value is not None:
              #set value as string text representing json
...
#class definition to access DB table business_connection
class BusinessConnection(db.Model):
	__tablename__ = 'business_connection'
     #class properties which are actual columns in table
    ...
    #function to format column values in custom way
	def full_name(self, spaced=True):
		if spaced:
              #return spaced first and last name
	...
	def generate_fake(count=100, **kwargs):
		...
		seed()
         #loop and generate fake business connections 
		for i in range(count):
              #instantiate BusinessConnection with fake/random values
...
#class definition to access DB table business_connection_search_type
class BusinessConnectionSearchType(db.Model):
	...
	def insert_search_types():
		...
		for n in names:
              #find search types list by given names
			search_type = BusinessConnectionSearchType.query.filter_by(name=n).first()
			if search_type is None:
                  #if no search type returned then use name instead 
				search_type = BusinessConnectionSearchType(name=n)
			db.session.add(search_type)
		db.session.commit()
#Followed this guide as it was hard to explain https://www.codecademy.com/en/forum_questions/551c137f51b887bbc4001b73
	def __repr__(self):
		return '<BusinessConnectionSearchType \'%s\'>' % self.id

#All below repeating

#class definition to access DB table business_connection_search_results
...
#class definition to access DB table business_connection_university
...
#class definition to access DB table business_connection_job
...
#class definition to access DB table business_connection_detail
